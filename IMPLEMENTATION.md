...
Schema additions in `supabase/schema.sql`: `packages.speed_limit/device_limit/mikrotik_profile/synced_to_router`, `vouchers.package_price/expires_at/mikrotik_user_synced/expired_at`, `transactions.amount_value/customer_phone/checkout_request_id/mpesa_receipt/provisioned_at/provider_ref`, `routers.api_port/api_user/last_synced_at/system_identity`, plus `stk_requests` and `audit_logs` tables. All statements are idempotent and safe to re-run.
