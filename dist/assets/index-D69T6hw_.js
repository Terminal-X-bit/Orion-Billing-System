...
`,ts=`/* Orion captive portal — standalone stylesheet.
...
`,ns=`// Orion captive portal client logic (no dependencies).
//
...
`,rs=`// Brand-color derivation shared by the bridge (portal HTML injection) and the
...
`;function is(e){let t={exports:{}};return Function(`module`,`exports`,`require`,e)(t,t.exports,()=>({})),t.exports}var as=is(rs);function os(e){return`<script>window.ORION_BRANDING=${JSON.stringify(e).replace(/</g,`\\u003c`)};<\/script>`}function ss(e){let t=as.buildBrandPalettes(e),n=e=>Object.entries(e).map(([e,t])=>`${e}: ${t};`).join(` `);return`<style>:root{${n(t.light)}}[data-theme="dark"]{${n(t.dark)}}</style>`}function cs(e){let t=es,n=ts,r=ns,i=os(e)+ss(e.primaryColor)+`
`,a=t.indexOf(`</head>`);return(a===-1?i+t:t.slice(0,a)+i+t.slice(a)).replace(/<link rel="stylesheet" href="portal\.css" \/>/,`<style>\n${n}\n</style>`).replace(/<script src="portal\.js"><\/script>/,`<script>\n${r}\n<\/script>`).replace(/<link rel="preconnect"[^>]*\/>\s*/g,``).replace(/<link rel="stylesheet" href="https:\/\/fonts\.googleapis\.com[^>]*\/>\s*/g,``)}var ls=is(`//---------------------------------------------------------------------
//
